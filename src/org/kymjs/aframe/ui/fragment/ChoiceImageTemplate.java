    ...
    class FolderListAdapter extends BaseAdapter {
        ...
        private void initData() {
            ...
            for (int i = 0; i < datas.size(); i++) {
                // 创建bitmap
            	Bitmap bitmap = BitmapCreate.bitmapFromFile(datas.get(i)
                        .getFilePath().get(0),
                        100, 100);// 指定尺寸为100
            	// li.dg
                //Bitmap bitmap = BitmapFactory.decodeFile(datas.get(i).getFilePath().get(0));
                if (bitmap != null) {
                    // 压缩bitmap
                    //bitmaps.add(BitmapHelper.scale(bitmap, 1, 1));
                	bitmaps.add(bitmap);
                } else {
